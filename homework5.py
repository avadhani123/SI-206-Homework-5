...
user_input = input("Enter a phrase to search: ")
if user_input in cache_dictionary:
	print("using cached data for", user_input)
	tweets = cache_dictionary[user_input]
else: 
	result = api.search(q = user_input)
	tweets = result["statuses"][:3]
	cache_dictionary[user_input] = tweets
	f = open(Cache_File,'w')
...
for tweet in tweets:
	text_string = "TEXT: " + tweet["text"] 
	f.write(text_string)
	f.write("\n")
	time_string = "CREATED AT: " + tweet["created_at"]
	f.write(time_string)
	f.write("\n")
	f.write("\n")
#### Recommended order of tasks: ####
