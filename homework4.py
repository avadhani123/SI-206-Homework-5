...
try:
  f = open(cache_filename,'r') #file opens
  text_data_from_site = f.read() 
  f.close()
except:
  r = requests.get("http://www.nytimes.com")
  text_data_from_site = r.text # receives html text attributes from site 
  f = open(cache_filename,"w") # opens up a writing file (one did not exist before)
  f.write(text_data_from_site) #writes file
  f.close()
